...
app.post("/students/add", (req, res) => {
  ...
  //validation for fields in add page
  if (!sid || sid.length !== 4) {
  ...
  mySQL_DAO
    ...
    .then((students) => {
      ...
      if (existingStudent) {
        ...
        //add student if passes validation
        mySQL_DAO
...
app.get("/students/edit/:sid", (req, res) => {
  ...
  mySQL_DAO
    .getStudents()
    .then((students) => {
      const student = students.find((s) => s.sid === sid); // find student by ID
      if (student) {
...
app.post("/students/edit/:sid", (req, res) => {
  ...
  // render page again with errors if fails
  if (errors.length > 0) {
...
//lecturers delete if no associated modules
app.get("/lecturers/delete/:lid", (req, res) => {
  var lecturerId = req.params.lid;

  //checl for modules
  mySQL_DAO.getModulesByLectID(lecturerId).then((modules) => {
    if (modules.length > 0) {
        //error message if has associated modules
      res.send(
        `<html> <head> <body> <a href="/lecturers">Home</a> <h1>Error message</h1><h3>Cannot delete lecturer ${lecturerId}. They have associated modules.<h3> </body></head></html>`
      );
    } else {
        //delete if no associated modules
